...
\label{sec:pipeline}
Our pipeline consists of CT-based affine pre-registration, LapIRN-based deformable registration, and optional test-time instance optimisation (Fig.~\ref{fig:pipeline}).
...
The levels are trained sequentially, with each finer level initialised from the previous one.
...
\subsubsection{Step 3: Instance optimisation.}
At test time, each pair is refined by optimising a residual stationary velocity field on top of the network prediction.
The residual is initialised to zero and exponentiated by scaling and squaring, preserving diffeomorphism while only correcting the predicted field.
Optimisation uses the training objective (Sec.~\ref{sec:losses}); each iterate is scored by DSC, HD95, MTV and TLG, and the best-scoring iterate is retained.
