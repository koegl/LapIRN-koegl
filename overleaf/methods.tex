...
\label{sec:container}
The submitted container must produce all labels required by instance optimisation and register each pair within the 90~s time limit.
It therefore differs from the validation configuration only in this final stage: PET lesion masks are predicted by a nnU-Net~\cite{nnunet} trained with progressive patch-size growing~\cite{ref_progressive_patch} on the challenge dataset, CT organ labels are generated with TotalSegmentator in fast mode, and instance optimisation runs until the time limit is reached.
...
Augmentations were chosen to mimic longitudinal variability: left--right flipping ($p=0.5$), CT intensity shifting by $\pm0.02$ in the normalised window (about $\pm50$~HU), CT scaling from $0.9$ to $1.1$, and PET scaling from $0.85$ to $1.15$ without additive shifts.
