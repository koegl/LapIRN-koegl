...
The two coarser levels (quarter and half resolution) use only the similarity, label and regularisation terms; thus $\lambda_{\mathrm{MTV}} = \lambda_{\mathrm{MTV\text{-}J}} = \lambda_{\mathrm{jac}} = \lambda_{\mathrm{TLG}} = \lambda_{\mathrm{rigid}} = 0$.
...
To additionally constrain the deformation within the lesion, we use $\mathcal{L}_{\mathrm{MTV\text{-}J}}$, which penalises deviations of the mean Jacobian determinant over the warped lesion region $W$ from one.
A third term, $\mathcal{L}_{\mathrm{jac}}$, penalises per-voxel deviations of the Jacobian determinant from one and therefore discourages local compensating compression and expansion.
...
Instance optimisation runs until the time limit is reached.
