"""
This file assumes the same imports as your existing training script:
torch, numpy as np, mlflow, plus your project's model/loss/transform
functions and TrainingConfig. Two new imports are required on top of that:

    import tqdm
    from typing import Dict

evaluate_lvl1 is a new helper that runs one no-grad pass over val_generator
and returns averaged losses. train_lvl1 now calls it every
config.val_interval steps, logs the results to mlflow under "val/", and
saves a separate "best" checkpoint whenever validation loss improves.
"""
...
def train_lvl1(
    ...
    grid_4 = generate_grid(config.imgshape_4)
    grid_4 = (
    ...
    while epoch <= config.epochs_lvl1:
        for X, Y, X_lbl_ct, X_lbl_pet, Y_lbl_ct, Y_lbl_pet in train_generator:
            ...
            if epoch % 100 == 0 or epoch == config.epochs_lvl1:
                ct = X_Y[:, 0:1, :, :, :]
                pet = X_Y[:, 1:2, :, :, :]
                my_data.save_volume(
                    volume=ct,
        ...
        if epoch > config.epochs_lvl1:
            break
        tqdm.tqdm.write("one epoch pass")
    pbar.close()
