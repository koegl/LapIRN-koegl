...
def train_lvl3(
    ...
    final_model_path = (
        config.model_save_dir
        / f"{config.mlflow_experiment}_{mlflow.active_run().info.run_name}_stagelvl3_{total_steps}.pth"
    )
    ...
    for global_step in range(start_global_step, total_steps):
        ...
        if config.overfit is True and saved_initial is False:
            ...
            my_data.save_volume(
                ...
                out_dir=config.save_dir / "initial",
                epoch=epoch,
                name=f"x_ref_ct_lvl3_{run_name}_{batch['case_id'][0]}",
            )
            my_data.save_volume(
                volume=x_affine[:, 0:1, ...],
                out_dir=config.save_dir / "initial",
                epoch=epoch,
                name=f"x_affine_ct_lvl3_{run_name}_{batch['case_id'][0]}",
            )
            my_data.save_volume(
                volume=Y_4x[:, 0:1, ...],
                out_dir=config.save_dir / "initial",
                epoch=epoch,
                name=f"y_ref_ct_lvl3_{run_name}_{batch['case_id'][0]}",
            )
            saved_initial = True
        ...
        if is_epoch_end:
            ...
            if config.overfit:
                print(
                    f"ep: {epoch} "
                    f"ncc={epoch_metrics['train_lvl3/ncc_ct'] / steps_per_epoch:.4f} "
                    f"dice={epoch_metrics['train_lvl3/dice_ct'] / steps_per_epoch:.4f}"
                )
