...
def train_lvl2(
    ...
    if config.use_lvl1 and config.warmup_epochs >= config.unfreeze_epoch_in_lvl2:
        print(
            ...
            f"head is still warming when lvl1 is unfrozen."
        )
    ...
    _flag = utils.stop_flag_path(config.save_dir, 2)
    _flag.parent.mkdir(parents=True, exist_ok=True)
    tqdm.tqdm.write(f"[lvl2] stop early with:  touch {_flag}")
    utils.log_text(f"touch {_flag}", "stop_lvl2_cmd.txt")
    ...
    for global_step in range(start_global_step, total_steps):
        ...
        train_metrics["train_lvl2/w_ncc_ct"] = config.w_ct * loss_ncc_ct.item()
        ...
        if loss_dice_ct is not None:
            train_metrics["train_lvl2/w_dice_ct"] = (
                config.w_dice_ct_lvl2 * loss_dice_ct.item()
            )
        if loss_dice_pet is not None and use_dice_pet:
            train_metrics["train_lvl2/w_dice_pet"] = (
        ...
        if config.overfit is False and (
            ...
            if valid_tubingen_generator is not None:
                ...
                utils.log_metrics(
                    {
                        f"valid_lvl2/val_{key}_tubingen": value
            ...
            if valid_nlst_generator is not None:
                val_losses_nlst = evaluate_lvl2(
                    model=model,
                    valid_generator=valid_nlst_generator,
                    config=config,
                    device=device,
                    loss_similarity_ct=loss_similarity_ct,
                    loss_similarity_pet=loss_similarity_pet,
                ...
                utils.log_metrics(
                    {
                        f"valid_lvl2/val_{key}_nlst": value
            ...
            if valid_abdomen_generator is not None:
                val_losses_abdomen = evaluate_lvl2(
                    model=model,
                    valid_generator=valid_abdomen_generator,
                    config=config,
                    device=device,
                    loss_similarity_ct=loss_similarity_ct,
                    loss_similarity_pet=loss_similarity_pet,
                ...
                utils.log_metrics(
                    {
                        f"valid_lvl2/val_{key}_abdomen": value
        ...
        if is_epoch_end and utils.check_stop_flag(config.save_dir, 2):
            tqdm.tqdm.write(f"[lvl2] stop flag at step {global_step} -> ending level")
            utils.log_metrics({"train_lvl2/early_stopped": 1.0}, step=global_step)
            break
