...
def load_data() -> List[Tuple[torch.Tensor, torch.Tensor]]:
    ...
    # print(f"{X_vol_pet.max()=}")
    # print(f"{Y_vol_pet.max()=}")

    # print(np.percentile(X_vol_pet, 99))
    # print(np.percentile(Y_vol_pet, 99))
...
def train_lvl1() -> str:
    ...
    while step <= iteration_lvl1:
        for X, Y in training_generator:
            ...
            if step % 100 == 0 or step == iteration_lvl1:
                ct = X_Y[:, 0:1, :, :, :]
                pet = X_Y[:, 1:2, :, :, :]
                save_volume(
                    ct,
...
def train_lvl2(path_model_level1: str) -> str:
    ...
    while step <= iteration_lvl2:
        for X, Y in training_generator:
            ...
            if step % 100 == 0 or step == iteration_lvl2:
                ct = X_Y[:, 0:1, :, :, :]
                pet = X_Y[:, 1:2, :, :, :]
                save_volume(
                    ct,
...
def train_lvl3(path_model_level2: str) -> None:
    ...
    while step <= iteration_lvl3:
        for X, Y in training_generator:
            ...
            if step % 100 == 0 or step == iteration_lvl3:
                ct = X_Y[:, 0:1, :, :, :]
                pet = X_Y[:, 1:2, :, :, :]
                save_volume(
                    ct,
